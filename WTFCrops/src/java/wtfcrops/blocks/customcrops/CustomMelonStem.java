...
public class CustomMelonStem extends CustomCropStem{

	public CustomMelonStem(int cultivar) {
		super(cultivar, Crops.blockMelon, Crops.melonStalk, Items.melon_seeds, Crops.itemMelon);
		this.baseGrowthRate = WTFCropsConfig.growthRateMelon;
	...
	public int getState(IBlockAccess world, int x, int y, int z)
	{
		...
		int gourdDir = -1;
		if (meta < 7) {return -1;}
		else if (world.getBlock(x - 1, y, z) instanceof CustomMelonBlock){gourdX=x-1;stemDir=0;gourdDir=3;}
		else if (world.getBlock(x + 1, y, z) instanceof CustomMelonBlock){gourdX=x-1;stemDir=1;gourdDir=0;}
		else if (world.getBlock(x, y, z-1) instanceof CustomMelonBlock){gourdZ=z-1;stemDir=2;gourdDir=0;}
		else if (world.getBlock(x, y, z+1) instanceof CustomMelonBlock){gourdZ=z+1;stemDir=3;gourdDir=2;}
	...
	protected void loadFarmlandHashMap(){
		...
		float badHydration = .5F;//this gets multiplied by the hydration value of the plant- it tells you how sensitive the plant is to the wrong hydration	
		int idealHydration = 9; //the hydration value where the plant is happiest, 0-15, 0 is dry, 15 is wet
		...
		for (int loop = 0; loop < 8; loop++){
			farmlandModifier.put(new BlockAndMeta(Crops.melonStalk, 0), 0F);
		}
	}

	@Override
	protected void loadBiomeHashMap() {
		biomeMods.put(Type.HOT, +2F);
